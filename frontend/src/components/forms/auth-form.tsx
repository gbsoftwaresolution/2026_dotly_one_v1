...
export function AuthForm({
  ...
  return (
    ...
    >
          ...
          {title ? (
            <div className="flex items-center justify-between gap-3">
              ...
              {collapsible ? (
                <button
                  type="button"
                  className="inline-flex items-center gap-2 rounded-full border border-black/5 bg-foreground/[0.03] px-3 py-1.5 text-xs font-semibold uppercase tracking-[0.16em] text-muted transition hover:bg-foreground/[0.05] dark:border-white/10 dark:bg-white/[0.05]"
      ...
      {mode === "signup" ? (
        <div className="rounded-[20px] border border-black/5 bg-foreground/[0.03] px-4 py-3 text-sm leading-6 text-muted dark:border-white/10 dark:bg-white/[0.05]">
          Create your account with email and password now. After that,
          Dotly&apos;s premium next step is adding a passkey for faster, calmer
          sign-in.
        </div>
      ) : null}

      {collapsible && !isExpanded ? null : (
